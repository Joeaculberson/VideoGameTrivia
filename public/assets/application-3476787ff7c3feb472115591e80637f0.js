...
function timerPowerUp() {
    if (isThereEnoughCoins(1)) {
        if (playSound === true) {
            var audio = new Audio('/assets/sounds/slowpoke-3f8850d4ecf1e70ba6fcba8317079aa0.mp3');
            audio.play();
        }
        currentTime += 1500;
        $.post('/pay',{amount:1});
...
function revealOnePowerUp() {
    if (isThereEnoughCoins(2)) {
        if (playSound === true) {
            var audio = new Audio('/assets/sounds/kerboom-40fb8989296129fb1910727de3be340f.wav');
            audio.play();
        }
        $.post('/pay',{amount:2});
        currentCoins -= 2;
...
function secondChancePowerUp() {
    if (isThereEnoughCoins(3)) {
        var audio = new Audio('/assets/sounds/fairy-20e9aff058bf1b2ea318ad7b0fb68e0d.wav');
        audio.play();
        $.post('/pay',{amount:3});
        currentCoins -= 3;
        updateCoins();
...
$(document).ready(function() {
    $(document).on('click', '.correct', function() {
        ...
        clearInterval(timerIntervalId);
        var audio = new Audio('/assets/sounds/success-2ff748795418d0cc1243bb137607a8ef.wav');
        audio.play();
    });
    ...
    $(document).on('click', '.incorrect', function() {
        ...
        var audio = new Audio('/assets/sounds/failure-c4d2e8f268edf05c1d6b415c11875ef7.wav');
        audio.play();
    });
...
function startSpin(determinedValue) {
    var audio = new Audio('/assets/sounds/wheel_ding-840feb2d7f7583499d34a956d741765b.wav');
    audio.play();
